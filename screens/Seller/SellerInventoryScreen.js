...
import useVendorProductsSearch from '../../hooks/useVendorProductsSearch'
    ...
    return (
        <View style={styles.container}>
            <ScrollView contentContainerStyle={{ flexGrow: 1 }} stickyHeaderIndices={[0]} showsVerticalScrollIndicator={false}>
                ...
                {isSearch ?
                    <Search query={search} field={searchBy} {...props} />
                    :
                    <Inventory {...props} />
                }
            </ScrollView>
...
function Search(props) {
    ...
    const { search_loading, search_error, search_products, search_hasMore, search_total } = useVendorProductsSearch(props.field, props.query, pageNumber, '10', props.token, props.user._id)
...
function Inventory(props) {
    const [pageNumber, setPageNumber] = useState(1)
    const { flats_loading, flats_error, flats_array, flats_pages, flats_total, flats_hasMore } = useFlatsInfiniteScroll(pageNumber, '10', props.token, props.user._id)
